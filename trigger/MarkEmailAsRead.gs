...
function triggerMarkEmailAsRead() {
  ...
  const analyzedAryObjEmail = aryObjEmail.map(objEmail => {
    ...
    // プロンプトの設定
    const prompt = '以下のメール本文を評価してください。リアクションが必要かどうかを0から10のスケールで数値で示し、リアクションが必要な理由とメール本文の要約を併せて指定されたJSONスキーマで出力してください。### 採点の要件、私の名前（山口征啓）が本文の最初に書いてあること。相手の名前も書いてある場合は加点してください'
      + '### メール本文:' + body + '\n'
